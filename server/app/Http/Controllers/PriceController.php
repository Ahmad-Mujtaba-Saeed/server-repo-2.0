...
class PriceController extends Controller
{
    ...
    public function GeneratedPaidFee(Request $request)
    {
        ...
        if ($GeneratedFee) {
            return response()->json(['success' => true, 'data' => $GeneratedFee, 'YearlyTotalFee' => ($YearlyTotalFee)]);
        } else {
    ...
    public function GeneratedChallans(Request $request)
    {
        $user = $request->user();
        if ($user->role != 'Admin') {
            return response()->json(['success' => true, 'message' => 'Only admin can see fee information']);
...
$expensives = expensives::whereYear('Date', $currentYear)
    ...
    ->get();
        ...
        if ($TeacherPay) {
            return response()->json(['success' => true, 'TotalExpensive' => ($TotalExpensive), 'combinedResults' => $combinedResults]);
        } else {
            return response()->json(['success' => true, 'message' => 'Error Fetching Fee information']);
        }
    }
